    ...
    * Role assignments:
    ...
    def _get_children(self, project_ids):
        ...
        return [ref for ref in project_refs
                if ref['parent_id'] in project_ids]
    ...
    def list_project_parents(self, project_id):
        project_ref = self.get_project(project_id)
        hierarchy = []
        while project_ref['parent_id'] is not None:
            parent_project = self.get_project(project_ref['parent_id'])
            ...
            try:
                ...
                if CONF.os_inherit.enabled:
                    ...
                    for role in data.get('roles', []):
                        if role.get('inherited_to', '') == 'projects':
                            # If there is any inherited role, add the current
                            # project subtree as projects for this user
                            project_ids.update(
                                x['id']
                                for x in self.get_project_subtree(project_id))
                            break
            except exception.NotFound:
