...
class ProjectV3(controller.V3Controller):
    ...
    def get_project(self, context, project_id):
        ...
        if ('parents' in context['query_string'] and
                self._query_filter_is_true(
                    context['query_string']['parents'])):
            parents = self.assignment_api.list_project_parents(project_id)
            ref['parents'] = [ProjectV3.wrap_member(context, p)
                              for p in parents]

        if ('subtree' in context['query_string'] and
                self._query_filter_is_true(
                    context['query_string']['subtree'])):
            subtree = self.assignment_api.get_project_subtree(project_id)
            ...
            if 'OS-INHERIT:inherited_to' in r['scope']:
                ...
                if 'domain' in r['scope']:
                    ...
                    project_ids = (
                        [project_id] +
                        [x['id'] for x in
                            self.assignment_api.get_project_subtree(
                                project_id)])
                    base_entry = copy.deepcopy(r)
